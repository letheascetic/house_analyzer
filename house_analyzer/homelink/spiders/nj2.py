...
class Nj2Spider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        super(Nj2Spider, self).__init__(*args, **kwargs)
    ...
    def _init_start_urls(self):
        for city, community in self.sql_helper.get_all_communities():
            if city == 'nj':
                self.start_urls.extend(['https://nj.lianjia.com/ershoufang/pg{0}rs{1}/'.format(page, community) for page in range(1, 11)])
                self.start_urls.extend(['https://nj.lianjia.com/chengjiao/pg{0}rs{1}/'.format(page, community) for page in range(1, 11)])
